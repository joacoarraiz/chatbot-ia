...
-- Correr en Supabase SQL Editor después de los archivos db/.
-- =====================================================

-- ----- rpc_buscar_producto -----
-- Full-text search sobre descripción, con ranking.
...
LANGUAGE sql STABLE AS $$
  SELECT
    ...
    p.marca_pieza,
    p.linea,
    COALESCE(SUM(o.stock), 0) AS stock_total,
    MIN(o.precio) FILTER (WHERE o.stock > 0) AS precio_min,
    ts_rank(to_tsvector('spanish', p.descripcion),
            plainto_tsquery('spanish', p_query)) AS match_score
  ...
  GROUP BY p.id
  ORDER BY match_score DESC, stock_total DESC
  LIMIT p_limit;
$$;


-- ----- rpc_buscar_por_aplicacion -----
-- La búsqueda más usada: por vehículo.
...
LANGUAGE sql STABLE AS $$
  ...
  ORDER BY stock_total DESC, precio_min ASC NULLS LAST
  LIMIT p_limit;
$$;
...
LANGUAGE sql STABLE AS $$
  ...
  ORDER BY e.confianza DESC, stock_total DESC;
$$;


-- ----- rpc_consultar_precio -----
-- Aplica reglas de precio (especificidad estilo CSS).
-- producto=100, marca=50, linea=30, global=0.
...
BEGIN
  ...
  SELECT * INTO v_regla
   ...
   WHERE r.empresa_id = p_empresa_id
     AND r.activa = true
     AND (
       (r.ambito = 'producto' AND r.ambito_valor = p_producto_id::TEXT)
  ...
  IF v_regla IS NULL THEN
    -- Sin regla: precio = oferta más barata con stock
  ...
  -- Aplicar estrategia de la regla
  IF v_regla.estrategia = 'menor_precio' THEN
    SELECT o.id, o.precio, o.fuente_id INTO v_oferta
      FROM oferta o
     WHERE o.producto_id = p_producto_id
       AND (NOT v_regla.requiere_stock OR o.stock > 0)
     ORDER BY o.precio ASC LIMIT 1;
  ...
  ELSIF v_regla.estrategia = 'lista_fija' AND v_regla.fuente_preferida_id IS NOT NULL THEN
    SELECT o.id, o.precio, o.fuente_id INTO v_oferta
      FROM oferta o
     WHERE o.producto_id = p_producto_id
       AND o.fuente_id = v_regla.fuente_preferida_id
       AND (NOT v_regla.requiere_stock OR o.stock > 0)
     LIMIT 1;
  END IF;

  -- Fallback si la regla no encontró nada
  IF v_oferta IS NULL THEN
    SELECT o.id, o.precio, o.fuente_id INTO v_oferta
      FROM oferta o
     WHERE o.producto_id = p_producto_id AND o.stock > 0
