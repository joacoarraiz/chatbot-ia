"""
agents/derivacion/derivacion.py
Crea una derivación a humano con resumen del contexto.
La derivación queda registrada en la tabla `derivacion` de Supabase.
"""
...
TOOLS_DEF = [
    {
        ...
        "function": {
            "name": "derivar_humano",
            "description": "Crea una derivación a humano en la base. El resumen_contexto es CRÍTICO porque el vendedor lo lee antes de tomar la conversación.",
            "parameters": {
                "type": "object",
                "properties": {
                    "consulta_id": {"type": "integer", "description": "ID de la consulta que se está derivando. Si no se tiene, usar 0."},
                    "motivo": {"type": "string", "description": "Razón breve. Ej: 'cliente pidió humano', 'catálogo incompleto', 'reclamo'."},
                    "resumen_contexto": {"type": "string", "description": "Resumen detallado: quién es el cliente, qué busca, qué se intentó, por qué se deriva."},
                    "valor_cotizado": {"type": "number", "description": "Valor estimado en pesos si ya se cotizó algo. Opcional."},
                },
                "required": ["consulta_id", "motivo", "resumen_contexto"],
            },
        },
    },
]
